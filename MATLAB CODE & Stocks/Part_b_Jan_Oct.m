...
for fileIndex = 1:length(csvFiles)
    ...
    % Filter out dates not in the range January 2011 to July 2011
    Data_Stock_Features = Data_Stock_Features(Data_Stock_Features.Date >= datetime(2011, 1, 1) & Data_Stock_Features.Date <= datetime(2011, 10, 31), :);
    ...
    Variances(fileIndex) = var(monthlyReturns, 1); %n-1 since using "var" by default divides by "n". This corrects the bias.
    STD(fileIndex) = std(monthlyReturns, 1);
end

% Compute the sample covariance matrix
allMonthlyReturns = cell2mat(monthlyReturnsAllAssets')'; % Convert cell array to matrix
Covariances = cov(allMonthlyReturns, 1); %n-1 since using "cov" by default divides by "n". This corrects the bias.
...
six_month_rates = [0.10];

% Market capitalization for all 20 assets in billions for 2011
market_cap = [40.88, 58.58, 67.36, 102.65, 158.91, 103.73, 204.65, 76.92, 145.33, 126.34, 377.51, 216.72, 166.34, 179.08, 406.27, 20.59, 18.16, 179.20, 113.58, 29.29];
